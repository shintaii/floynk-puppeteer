...
exports.SEARCH_PAGE = async (
  ...
  const data = await page.evaluate(
    (maxPostCount, query, savedItems) => {
      ...
      let results = Array.from(
        document.querySelectorAll(".sh-dlr__list-result")
      );
      if (results.length === 0)
        results = Array.from(document.querySelectorAll(".sh-dgr__content"));
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const productLinkAnchor = item.querySelector(
        ...
        const price =
          item.querySelector('div[data-sh-or="price"] div > span > span')
            ?.textContent ?? null;

        const shippingCost =
          item.querySelector("div.vEjMR")?.textContent ?? null;
        // const description = item.querySelectorAll('div.hBUZL')[1]?.textContent ?? null;

        const merchantName =
          item.querySelector('div[data-sh-or="price"]')?.nextSibling
            ?.textContent ?? null;
        ...
        const idArray = productLink
          ? productLink.split("?")[0].split("/")
          : null;
        const shoppingId = idArray ? idArray[idArray.length - 1] : null;

        const reviewInfo = item.querySelector('div[aria-label*=","]')
          ? item
        ...
        const output = {
          ...
          productDetails: item
            .querySelectorAll(".translate-content")[1]
            ?.textContent.trim(),
        };
  ...
  log.info(
    `${Math.min(
      maxPostCount,
      resultsLength
    )} items on the page were successfully scraped.`
  );
};
